...
class Database {
    constructor() {
        const storageChart = JSON.parse(localStorage.getItem("shoppingChart"));
        this.products = [];
    ...
    findProductByName(name) {
        ...
        return this.products.filter((product) => product.name.toLowerCase().includes(name));
    }
...
class ShoppingCart {
    constructor() {
        const storageChart = JSON.parse(localStorage.getItem("shoppingChart"));
        this.shoppingCart = storageChart || [];
        this.total = 0;
    ...
    addProductToTheCart(product) {
        ...
        if (productInCart) {
            ...
            localStorage.setItem("shoppingChart", JSON.stringify(this.shoppingCart));
        }
        this.loadShoppingCart();
    }
    deleteProduct(id) {
...
inputSearch.addEventListener("keyup", (event) => {
    ...
    console.log("quero buscar:" + productName)
    const products = db.findProductByName(productName.toLowerCase());
